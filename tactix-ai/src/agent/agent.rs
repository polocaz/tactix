...
pub struct PerceptionData {
    pub visible_enemies: Vec<u32>, // List of IDs
    pub nearest_cover: Option<Vec2>,
    pub is_under_fire: bool,
}
    ...
    pub fn apply_action(&mut self, action: AgentAction) {
        match action {
            ...
            AgentAction::Attack(target_id) => {
                // For Phase 0, we won't implement attack logic
                // Just change state to Attacking
                self.state = AgentState::Attacking(target_id);
            }
            AgentAction::Idle => {}
        }
    }
